...
# imports
from typing import Iterator, Callable, List, Optional, Any, Coroutine
...
async def process_api_requests_from_file(
    ...
    with open(requests_filepath) as file:
        ...
        requests = map(lambda x: json.loads(x), file.__iter__())

        async def callback(error: Exception, request_json: str, response, metadata: Any) -> None:
            data = [request_json, response, metadata]
            append_to_jsonl(data, save_filepath)

        status_tracker = await process_api_requests_from_iterator(
            requests, callback, request_url, api_key, max_requests_per_minute, max_tokens_per_minute, token_encoding_name, max_attempts, logging_level,
        )
...
async def process_api_requests_from_iterator(
    requests: Iterator,
    callback: Callable[[Optional[Exception], str, List[str], Optional[Any]], Coroutine],
    request_url: str,
    api_key: str,
    max_requests_per_minute: float,
    max_tokens_per_minute: float,
    ...
    status_tracker = (
        StatusTracker() if not status_tracker else status_tracker
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(
        ...
        while True:
            ...
            if (
                ...
                logging.debug(
                    f"await asyncio.sleep({remaining_seconds_to_pause})"
                )
...
class APIRequest:
    ...
    async def call_api(
        ...
        if error:
            ...
            logging.debug(f"Request {self.task_id} successful")
...
def num_tokens_consumed_from_request(
    ...
    if api_endpoint.endswith("completions"):
        ...
        logging.error(
            f"""calculating num_tokens_consumed_from_request completion_tokens={completion_tokens}"""
        )
        ...
        if api_endpoint.startswith("chat/"):
            ...
            logging.error(
                f"""calculated num_tokens_consumed_from_request num_tokens={num_tokens}"""
            )
